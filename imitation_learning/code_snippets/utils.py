...
def sample_trajectory(env, policy, max_path_length, render=False):
    ...
    reset_out = env.reset() 
    ob = reset_out[0] if isinstance(reset_out, tuple) else reset_out # HINT: should be the output of resetting the env
    ...
    while True:
        ...
        ac = np.asarray(ac, dtype = np.float32).reshape(-1)
        acs.append(ac)
        # Take that action and record results
        step_out = env.step(ac) 
        if len(step_out) == 5: 
            ob2, rew, terminated, truncated = step_out
            done = terminated or truncated
        else: 
            ob2, rew, done, _ = step_out
