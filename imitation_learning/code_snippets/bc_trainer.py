...
class BCTrainer:
    ...
    def collect_training_trajectories(
        ...
        if itr == 0 and load_initial_expertdata is not None:
            print('\nLoading expert data from: {}'.format(load_initial_expertdata))
            with open(load_initial_expertdata, 'rb') as f:
                expert_data = pickle.load(f)
            paths = expert_data
            envsteps_this_batch = sum(len(p['reward']) for p in expert_data)
            print('Loaded {} paths consisting of {} timesteps'.format(
                len(paths), envsteps_this_batch))
        else:
            print('\nCollecting new data using the current policy: {}'.format(collect_policy))
            paths, envsteps_this_batch = utils.sample_trajectories(
                self.env, collect_policy,
                self.params['batch_size'],
                self.params['ep_len']
            )
            print('Collected {} paths consisting of {} timesteps'.format(
                len(paths), envsteps_this_batch))
        ...
        if self.log_video:            
            ...
            train_video_paths = utils.sample_n_trajectories(self.env,
                collect_policy, MAX_NVIDEO, self.params['ep_len'], True)
